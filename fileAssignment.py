...
class MiniBank:
    ...
    def dataReading(self):
        with open("userInfo.txt", 'r') as fileRead:
            ...
            for i in contents:
                count = len(self.main_userInfo) + 1
                id = count
                print("Id testing",id,type(id))
    ...
    def dataoverWriting(self, email, password, amount):
        try:
            with open("userInfo.txt", 'w') as fileoverWrite:
                data = email + " " + password + " " + amount + "\n"
                fileoverWrite.write(data)
                print("\n[+]Data Overwrited Successfully to File!\n")
                fileoverWrite.close()
        except Exception as err:
            print("\n [#] Error Logs From FileWriting\n")
            print(err)
    ...
    def login_checking(self, l_username, l_pwd):
        ...
        print("\n\nfrom login checking funciton:", userInfo_length)
        print(self.main_userInfo)
    ...
    def updateName(self,loginId):
        self.dataReading()
        id = loginId
        name = self.main_userInfo[id]["username"]
        password = self.main_userInfo[id]["password"]
        amount = self.main_userInfo[id]["amount"]
        nName = input("Please enter your new name")

        self.main_userInfo[id]["username"]=nName
        # #file open
        # with open("userInfo.txt","r") as file:
        # self.dataoverWriting(nName,password,amount)
        ...
        # file.truncate(0)
        # file.close()
        self.dataWriting(nName,password,amount)
    ...
    def updatePwd(self,loginId):
        ...
        self.main_userInfo[id].update({"username": nPwd})
        self.dataoverWriting(name, nPwd, amount)
